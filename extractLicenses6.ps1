...
# If they do it captures what licenes and how long ago did the user logon
# If no sign-in activity checks if it was just created
# Also extracts complementary data including OU structure regex (thx to chat gpt!)

$DateTime = Get-Date
$DateTimeString = $DateTime.ToString("yyyyMMddHHmmss")
$JustCreated  =15  #Number of days to consider account has just been created
$InactiveDays =90  #Number of days to consider account is not being used
...
Write-Output((Get-Date -Format "HH:mm:ss") + " Gather Licenses (ETA 3h)")
foreach ($User in $AllUsers) {
    ...
    $UserOU =if ($UserDN -match 'OU=([^,]+),OU=([^,]+)') {$matches[2]} else {"Mismatch"}
    #$UserOU=if ($UserDN -match 'OU=Users,OU=([^,]+)') {$matches[1]} else {"Mismatch"}
    #$UserOU=$UserDN.Substring($UserDN.IndexOf(",OU=UnilabsGroup,DC=uni,DC=ad") - 2, 2)
    ...
    if ($Licenses -ne $null) {
        ...
        $signInLogs   = (Get-MgUser -Filter "UserPrincipalName eq '$($user.UserPrincipalName)'" -Property SignInActivity).SignInActivity.LastSignInDateTime
        if ($signInLogs) {
            # Here user has signed-in at least once, let's make a note of it
            Write-Host "(UserID: $($user.Objectid)) Last sign-in time for user $($user.UserPrincipalName): $($signInLogs)"
            $LastSignIn=($DateTime - $signInLogs).days
        } else {
